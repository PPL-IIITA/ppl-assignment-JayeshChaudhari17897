...
def cmp_attractiveness (b1 , b2 ):
	if b1.happiness > b2.happiness :
		return 1
	elif b1.happiness < b2.happiness :
		return -1
	else :
		return 0


def cmp_maintenance ( g1 , g2 ):
	if g1.maintenance > g2.maintenance :
		return 1
	elif g1.maintenance < g2.maintenance :
		return -1
	else :
		return 0
...
boy_array.sort(cmp_attractiveness , reverse = False )
girl_array.sort(cmp_maintenance , reverse = False)
...
while cnt<row_count_girls  :
	...
	if flag == 0 :
		for j in range(row_count_girls):
			...
			for j in range(row_count_boys):
				for i in range(row_count_girls):
					if (boy_array[j].budget >= girl_array[i].maintenance and boy_array[j].attractiveness <= girl_array[i].attractiveness  and boy_array[j].committed==False and girl_array[i].committed == False) :
						boy_array[j].committed = True
						girl_array[i].committed = True
						couple_array.append(couple(boy_array[j],girl_array[i]))
						flag = 1
					break
