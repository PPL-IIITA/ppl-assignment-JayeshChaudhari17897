...
for j in range(row_count_girls):
	for i in range(row_count_boys):
		best_k_boys=template()
		best_k_boys.best_k(row_count_boys , boy , boy_array)#here the function sorts according to value of attarctiveness and provides best k here we are taking k as count of boys  , if needed change the value of k in the arguement 
...
with open("ans_file_couples_q7.txt","w") as ansfile :
...
for i in range(k):
	gift_store = template()
	gift_store.best_k(row_count_gifts , gift , gift_array)
	if (couple_array[i].boy.typee == 'Geek'):
		...
		while (i < main_cost):
			...
			file = open('gifting_log_q9.txt','a') 
			file.write(couple_array[i].boy.name +' gifted to   ' + couple_array[i].girl.name+'\n') 
			file.close()
			y = y + 1
